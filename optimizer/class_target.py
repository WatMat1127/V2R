...
class Target:
    ...
    def make_input_files(self, dat_info):
        ...
        if os.path.isfile(ftop_new + '.com'):
            ...
            if nstep >= 1:
                ...
                subprocess.run('cp {0}_MO.rrm_ex {1}_MO.rrm_ex'.format(ftop_best, ftop_new), shell=True)
    ...
    def copy_result(self, nstep, istep):
        ...
        subprocess.run('cp {0}_MO.rrm_ex {1}_MO.rrm_ex'.format(fn_istep, fn_nstep), shell=True)
    ...
    def read_QM_struct_ene(self, nstep, ene_tag):
        path_com = self.fn_target + '_step{}.com'.format(nstep)
        path_log = self.fn_target + '_step{}.log'.format(nstep)

        com_file = class_general.ComFile(path_com)
        com_file.read_grrm_com()
        tag_max_itr = False
        for ioption in com_file.option:
            if "max" in ioption.lower() and "itr" in ioption.lower():
                tag_max_itr = True
                max_itr = int(ioption.split("=")[1])
        ...
        while wait_tag:
            if os.path.isfile(path_log):
                with open(path_log, mode='r') as f:
                    ...
                    itr_idx = [i for i, line in enumerate(list) if '# ITR.' in line]

                    if len(struct_idx) == 1 and len(ene_idx) == 1:
                        wait_tag = False

                    else:
                        if tag_max_itr and len(struct_idx) == 0:
                            n_itr = int(list[itr_idx[-1]].split()[2])
                            if max_itr == n_itr:
                                wait_tag = False
                            else:
        ...
        log_file.struct.geom2list()
        if ene_tag == 'E':
            ene = log_file.struct.e_ene
        elif ene_tag == 'G':
            ene = log_file.struct.g_ene

        self.latest_struct = log_file.struct.geom_list
    ...
    def calc_grad(self, dat_info):
        ...
        if os.path.isfile(path_param_grad):
            ...
            subprocess.run('python {0} {1} {2} {2} param_grad'.format(path_sterep, path_LinkJOB, path_LinkJOB.split('_LinkJOB.rrm')[0]), shell=True)
        grad = []
