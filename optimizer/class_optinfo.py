...
class OptInfo:
    ...
    def function(self):
        ...
        # copy result if the calculations have just been done
        if len(param_idx) >= 2 and param_idx[-1] == self.nstep and param_idx[-2] == self.nstep - 1:
           istep = param_idx[-2]
           for itgt in target_list:
    ...
    def calc_initSS(self):
        ...
        self.make_reference_point()

    def make_reference_point(self):
        self.nstep_best = self.nstep  # make reference point
        for i in range(len(self.target_list)):
            self.target_list[i].nstep_best = self.nstep
    ...
    def linesearch_safe(self):
        ...
        for jitr in range(1000):
            ...
            #####-----
            # correct too large ss for next step
            #####-----
            tmp_vec = ss * d_vec
            max_idx = 99
            max_ratio = -10
            ...
            for iparam in range(len(tmp_vec)):
                iparam_min = self.param_range[iparam][0]
                iparam_max = self.param_range[iparam][1]

                iparam_ratio = np.abs(tmp_vec[iparam]) / (iparam_max - iparam_min)

                if iparam_ratio > max_ratio:
                    max_idx = iparam
                    max_ratio = iparam_ratio
                    max_param_range = iparam_max - iparam_min

            if max_ratio > 0.1:
                ss = 0.1 * max_param_range / np.abs(d_vec[max_idx]) * np.sign(ss)
