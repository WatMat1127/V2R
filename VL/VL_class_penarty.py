...
class Penarty:
    ...
    def add_keep_pot(self, xyz, keep_info):
        if len(keep_info) != 0:
            ...
            grad = torch.func.jacrev(VL_keep_pot.calc_keeppot)(xyz, keep_info)
            hessian = torch.func.jacrev(torch.func.jacrev(VL_keep_pot.calc_keeppot))(xyz, keep_info)
    ...
    def add_keep_pyr_pot(self, xyz, keep_pyr_info):
        if len(keep_pyr_info) != 0:
            ...
            grad = torch.func.jacrev(VL_keep_pyr_pot.calc_keep_pyr_pot)(xyz, keep_pyr_info)
            hessian = torch.func.jacrev(torch.func.jacrev(VL_keep_pyr_pot.calc_keep_pyr_pot))(xyz, keep_pyr_info)
            self.add_ene = ene.item()
            self.reshape_torch_grad_hess(grad, hessian)
    ...
    def add_LJ_asym_ell_pot(self, xyz, LJ_asym_ell_info, MM_param_list, path_phi_log, atom_order):
        if len(LJ_asym_ell_info) != 0:
            ...
            self.add_ene = ene.item()
            del ene
            grad = torch.func.jacrev(VL_LJ_asym_ell_pot.calc_ene)(xyz_phi_tensor, MM_param_list, LJ_asym_ell_info)
            hessian = torch.func.jacrev(torch.func.jacrev(VL_LJ_asym_ell_pot.calc_ene))(xyz_phi_tensor, MM_param_list, LJ_asym_ell_info)
