...
class LinkJOB:
    def __init__(self, path):
        ...
        with open(path, mode='r') as f:
            ...
            mofile_i = [i for i, line in enumerate(fdat_linkjob_list) if 'MO     =' in line]
            ...
            # load MO file name
            fn_mo = fdat_linkjob_list[mofile_i[0]].split()[2]
    ...
    def write_LinkJOB(self, path_LinkJOB_write):
        ...
        raw_energies = [float(i) for i in fdat_linkjob_list[self.energy_index].split()[2:]]
        add_ene = self.ene - raw_energies[0]
        out_ene = [self.ene, raw_energies[1], raw_energies[2]]
        if out_ene[1] != 0.0:
            out_ene[1] = out_ene[1] + add_ene
        if out_ene[2] != 0.0:
            out_ene[2] = out_ene[2] + add_ene

        fdat_linkjob_list_write = copy.deepcopy(fdat_linkjob_list)
        fdat_linkjob_list_write[self.energy_index] = f"ENERGY = {out_ene[0]: 17.12f} {out_ene[1]: 17.12f} {out_ene[2]: 17.12f}"
        for iatom in range(natom):
...
class ComFile:
    def __init__(self, path):
        with open(path, mode='r') as f:
            ...
            jobtype_index = [i for i, line in enumerate(fdat_com_list) if '#' in line]
            ...
            end_index = [i for i, line in enumerate(fdat_com_list) if 'options' in line.lower() or 'product' in line.lower()]
            option_index = [i for i, line in enumerate(fdat_com_list) if 'options' in line.lower()]
            jobtype = fdat_com_list[jobtype_index[0]].split('#')[1].split('/')[0].strip().lower()
            ...
            if len(frozen_index) == 0:
                ...
                self.options = fdat_com_list[option_index[0]:]
        ...
        with open(path, mode='r') as f:
            ...
            for iligand in range(nligand):
                ...
                for ipot in range(nLJ_asym_ell):
                    ...
                    iLJ_asym_ell_info['n_val'] = n_val
                    iLJ_asym_ell_info['search_method'] = search_method
...
class PhiLog:
    def __init__(self, path):
        ...
        self.n_phi_search = n_phi_search