...
def calc_ene(xyz_phi_tensor, MM_param_list, LJ_asym_ell_info, log_tag=False, atom_order=None, path_phi_log=None):
    ...
    for ilig in LJ_asym_ell_info:
        ...
        for ipot in LJ_asym_ell_info[ilig]:
            ...
            vec_p_1 = vec_1 - vec_p
            vec_p_origin = vec_p_1 / torch.linalg.norm(vec_p_1) * dist_p_origin
            vec_origin = vec_p_origin + vec_p
            vec_p_std = torch.cross(vec_p_1, vec_p_axis) / torch.linalg.norm(torch.cross(vec_p_1, vec_p_axis))
    ...
    for ipot_num in dat_pot_num:
        ...
        c1_val = LJ_asym_ell_info[ilig][ipot]["c1_val"]
        c2_val = LJ_asym_ell_info[ilig][ipot]["c2_val"]
        n_val_1 = LJ_asym_ell_info[ilig][ipot]["n_val"]
        target_atoms = LJ_asym_ell_info[ilig][ipot]['target_atoms']

        sgm_a1 = 2 * 2 ** (1 / n_val_1) * a1_val
        sgm_a2 = 2 * 2 ** (1 / n_val_1) * a2_val
        sgm_b1 = 2 * 2 ** (1 / n_val_1) * b1_val
        sgm_b2 = 2 * 2 ** (1 / n_val_1) * b2_val
        sgm_c1 = 2 * 2 ** (1 / n_val_1) * c1_val
        sgm_c2 = 2 * 2 ** (1 / n_val_1) * c2_val
        ...
        for iatom in range(len(target_atoms)):
            ...
            n_val = (n_val_1 * n_val_2) ** 0.5

            ene = epsilon * ((1.0 / dist_norm) ** (2 * n_val) - 2 * (1.0 / dist_norm) ** n_val)
        ...
        for jpot_num in dat_pot_num:
            if jpot_num <= ipot_num:
                ...
                n_val_2 = LJ_asym_ell_info[jlig][jpot]["n_val"]

                sgm_a1_jpot = 2 * 2 ** (1 / n_val_2) * a1_val_jpot
                sgm_a2_jpot = 2 * 2 ** (1 / n_val_2) * a2_val_jpot
                sgm_b1_jpot = 2 * 2 ** (1 / n_val_2) * b1_val_jpot
                sgm_b2_jpot = 2 * 2 ** (1 / n_val_2) * b2_val_jpot
                sgm_c1_jpot = 2 * 2 ** (1 / n_val_2) * c1_val_jpot
                sgm_c2_jpot = 2 * 2 ** (1 / n_val_2) * c2_val_jpot
    ...
    if log_tag:
        path_view = path_phi_log.split('.phi_log')[0].split('.rrm')[0] + '_ovoid.xyz'
...
def do_phi_opt(xyz, phi_list, MM_param_list, LJ_asym_ell_info, result_dat):
    phi_list_tmp, CG_tag = phi_CG_opt(xyz, phi_list, MM_param_list, LJ_asym_ell_info, [-4, -4, -1])
    phi_list_tmp = phi_list_tmp.tolist()
    phi_list_tmp = torch.tensor(phi_list_tmp, requires_grad=False, dtype=torch.float64)
    ...
    result_dat[tmp_add_ene.item()] = phi_list_tmp.tolist()
...
def phi_system_search(xyz, phi_list, MM_param_list, LJ_asym_ell_info, n_pot, n_step=6):
    ...
    for ipot in range(n_pot):
            ...
            for jstep in range(n_step):
                ...
                process = Process(target=do_phi_opt, args=(xyz, phi_list_tmp, MM_param_list, LJ_asym_ell_info, result_dat))
                process.start()
            ...
            if min(result_dat) < min_score:
                min_score = min(result_dat)
                phi_min = result_dat[min(result_dat)]

            phi_list = torch.tensor(phi_min, requires_grad=False, dtype=torch.float64)
    return min_score, phi_list
...
def phi_newton_opt(xyz, phi_list, MM_param_list, LJ_asym_ell_info):
    ...
    while True:
        ...
        grad = torch.func.jacrev(calc_ene_phi_tensor, argnums=1)(xyz, phi_list, MM_param_list, LJ_asym_ell_info)
        hessian = torch.func.jacrev(torch.func.jacrev(calc_ene_phi_tensor, argnums=1), argnums=1)(xyz, phi_list, MM_param_list, LJ_asym_ell_info)
...
def phi_CG_opt(xyz, phi_list, MM_param_list, LJ_asym_ell_info, threshold_list=[-8,-10,-7]):
    ...
    f_grad_n = torch.func.jacrev(calc_ene_phi_tensor, argnums=1)(xyz, phi_list, MM_param_list, LJ_asym_ell_info)
    maxitr = 50
...
def linesearch_safe(xyz, x_vac_n, MM_param_list, LJ_asym_ell_info, d_vec, f_val_n, f_threshold, a_threshold):
    ss = torch.tensor(0.01, dtype=torch.float64)
    ...
    for jitr in range(100):
        ...
        f_grad_n = torch.func.jacrev(calc_ene_phi_tensor, argnums=1)(xyz, x_vac_n, MM_param_list, LJ_asym_ell_info)
...
def microiteration_phi(xyz, MM_param_list, LJ_asym_ell_info, path_phi_log):
    ...
    if os.path.isfile(path_phi_log):
        ...
        itr_num = phi_log.n_phi_search
        n_pot = len(phi_list)
        # if (itr_num) % (n_pot * 2) != 0:
        if (itr_num) % 5 != 0:
            search_tag = False
        ...
        if search_tag:
            ...
            for ilig in LJ_asym_ell_info:
                for ipot in LJ_asym_ell_info[ilig]:
                    search_method = LJ_asym_ell_info[ilig][ipot]['search_method']
                    break
                break

            # global search
            if search_method == 'rot':
                tmp_min_score, tmp_phi_min = phi_system_search(xyz, phi_list, MM_param_list, LJ_asym_ell_info, n_pot)

            elif search_method == 'none':
                print('None')
                tmp_min_score = min_score

            else:
                print('[!] undefined search method: {}'.format(search_method))
                print('[!] exit...'.format(search_method))
                exit()

            # update phi_list if the score improved
            if tmp_min_score < min_score:
                phi_list = tmp_phi_min
                update_tag = True
    ...
    #####----------
    # Newton optimization of phi
    #####----------
    phi_list = phi_list.tolist()
    phi_list = torch.tensor(phi_list, requires_grad=False, dtype=torch.float64)
    ...
    while icount < 3:
        ...
        if newton_tag:
            ...
            phi_list, CG_tag = phi_CG_opt(xyz, phi_list, MM_param_list, LJ_asym_ell_info, [-3, -4, -1])
            icount += 1
    if not newton_tag:
        phi_list, CG_tag = phi_CG_opt(xyz, phi_list, MM_param_list, LJ_asym_ell_info, [-4, -4, -1])
    ...
    with open(path_phi_log, mode='a') as f:
        ...
        if newton_tag:
            f.write(' Newton opt succeeded')
        else:
            f.write(' Newton opt failed')
        if CG_tag:
            f.write(f' (after {icount} CG opt cycles)')
